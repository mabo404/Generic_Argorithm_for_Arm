classdef IK
    methods (Static)     
        function [theta, info] = IK_solver(L, Ptraj, Qtraj, theta0)
            ...
            maxIter = 150; % 反復・停止条件(小さい→到達前に打ち切り, 大きい→遅いが安定)
            tol_pos = 1e-5; % 収束閾値(大きい→すぐ収束（更新しないように見える）, 小さい→粘る（遅い／収束しないリスク）)
            tol_ori = 1e-4;
            maxStep = 30*pi/180; % 1反復の角度上限(小さい→常時クリップ→**dθ≈0 で"進まない"**症状, 大きい→収束早いがオーバーシュート増。45〜60°で緩めて様子見が定石)
            back_beta = 0.7; % 縮小率(小さい（例:0.5）→縮小速い＝受理されやすいが小刻み, 大きい（例:0.8）→なかなか縮まらず不採択連発の恐れ)
            back_max = 15; % 最大回数(小さい→試行不足で不採択→停滞, 大きい→試行過多で遅いが、受理ゼロは避けやすい)
            ...
            for t = 1:M
                ...
                for it = 1:maxIter
                    ...
                    if isfinite(rcond(JW)) && rcond(JW) > 1e-8
                        dth = JW \ eW;
                    else
                        dth = pinv(JW, 1e-8) * eW;
                    end
