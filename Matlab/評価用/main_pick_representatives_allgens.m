...
% 対象ファイルを収集
files = dir(fullfile(baseDir, 'gen_*_objs.csv'));
if isempty(files)
    error('フォルダに gen_*_objs.csv が見つかりません。');
end
...
for i = 1:numel(files)
    ...
    n = size(F,1);
    g = sscanf(files(i).name, 'gen_%d_objs.csv');  % 世代番号抽出
    F_all      = [F_all; F];
...
% 列名（必要に応じて合わせて下さい）
obj_names = ["J_tau","J_pos","J_ori","J_sc"];
