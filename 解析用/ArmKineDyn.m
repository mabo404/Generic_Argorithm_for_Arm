function [theta, info] = IK_solver(L, Ptraj, Qtraj, theta0)
    M = size(Ptraj,2);
    if ~(util_isfinite(L) && util_isfinite(Ptraj,'mat') && isa(Qtraj,'quaternion') ...
         && util_isfinite(theta0))
    ...
    for t = 1:M
        ...
        for it = 1:maxIter
            ...
            if accepted
                ...
                if F_cand < F_best && util_isfinite(F_cand)
        ...
        if any(strcmp(status, {'fkNaN','JNaN'}))
            if t < M, theta(:,t+1:end) = theta(:,t); end
...
function torque = computeTorque(L, theta, ddtheta, m_joint)
    if ~(util_isfinite(L) && util_isfinite(theta,'mat') && util_isfinite(ddtheta,'mat') && util_isfinite(m_joint))
        torque = nan(size(theta));
        warning('computeTorque:nonfiniteInput','Inputs contain NaN/Inf; returning NaN torque.');
        return;
    end
    [n, M] = size(theta);
    torque = zeros(n, M);
    ...
    for k = 1:M
        ...
        for ell = 1:n
            ...
            for j = 1:ell
                ...
                pp = fk_link_cog(L, tp, ell, c);
                pm = fk_link_cog(L, tm, ell, c);
                if ~(util_isfinite(pp,'vec3') && util_isfinite(pm,'vec3'))
                    tau_g(:) = NaN;                  % --- add
                    break;                            % --- add
                end
...
function p = fk_link_cog(L, theta, ell, c)
    if ~(util_isfinite(L) && util_isfinite(theta) && util_isfinite(c))
        p = [NaN;NaN;NaN]; return;
    end
    T = eye(4);
    for i = 1:ell
        if ismember(i, [1,4,6])
...
function dmin = minLinkDistance(L,theta)
    if ~(util_isfinite(L) && util_isfinite(theta,'mat'))
        dmin = inf; return;
    end
    [~,N] = size(theta);
    dmin = inf;
    for k=1:N
        ...
        for i=1:6
            ...
            if ~util_isfinite(T4,'mat') || ~isreal(T4)
                dmin = inf; return;
            end
            P(:,i+1) = T4(1:3,4);
        end
