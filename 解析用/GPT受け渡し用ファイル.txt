...
logdir = fullfile(pwd, "run_" + ts); % カレントフォルダ直下の出力先パス（'run_タイムスタンプ'）を組み立てる※まだ作成はしない
if ~isfolder(logdir) % 出力用ディレクトリが存在しなければ作成（初回実行時の安全策）
    mkdir(logdir);
end
dbgfile = fullfile(logdir,'debug_summary.csv'); % デバッグ用サマリCSVのファイルパス（各世代の概要を追記保存）
fid = fopen(dbgfile,'w'); % デバッグCSVを新規作成モードで開く（既存内容は破棄）。戻り値はファイル識別子 fid
fprintf(fid,'generation,gen_time_sec,invalid_count,viol_safety,best_J_tau,best_J_pos,best_J_ori,best_J_sc\n'); % デバッグCSVにヘッダ行（列名）をカンマ区切りで書き込む
fclose(fid); % 開いたファイルを閉じる（バッファを書き出してリソース解放）
...
nf = ~all(isfinite(pop_fitness), 2); % 非有限の行を検出
pop_fitness(nf, :) = INVALID_FITNESS; % その行は無効値に潰す
...
rank_now = pareto_rank(pop_fitness); % 初期集団に対するパレートランクを計算（1が最良フロント）
...
while generation < max_gen % 世代ループの開始条件（現在世代が最大世代に達するまで反復）
    ...
    rank = pareto_rank(pop_fitness); % 現在の集団に対してパレートランクを計算（1が最良フロント）
    cd_all = compute_crowding_all(pop_fitness, rank); % フロントごとに算出したクラウディング距離を、全個体分のベクトルとして集約する
    sel = tournament_select(tmp, rank, cd_all); % トーナメント選択で親個体のインデックスを決定（優先: ランク最小、同率はクラウディング距離が大きい方）
    parents = pop_vari(sel,:); % 選択された親個体の設計変数行列を抽出（行=親, 列=設計変数）
    
    % ===== [B] 交叉（SBX） =====
    pop_crossover = sbx_crossover(parents, lower_bound, upper_bound, dis_c); % SBX（模擬二進交叉）で子個体を生成（境界を尊重、分布指数 dis_c を使用）

    % ===== [C] 変異（多項式変異） =====
    pop_mutation = poly_mutation(pop_crossover, lower_bound, upper_bound, dis_m, pro_m); % 多項式変異で子個体に摂動を付与（分布指数 dis_m・変異確率 pro_m を使用）

    % ===== [D] 子個体の評価 =====
    pop_mutation_fitness = Evaluate(pop_mutation); % 生成した子個体の目的関数を評価（行=個体，列=[J_tau,J_pos,J_ori,J_sc]）
    nf = ~all(isfinite(pop_mutation_fitness), 2);
    pop_mutation_fitness(nf, :) = INVALID_FITNESS;
    ...
    [pop_vari, pop_fitness] = environmental_selection(combined_vari, combined_fit, pop_size); % 環境選択で合体集団から次世代を選抜（NSGA-II：ランク優先→クラウディング距離）
    pop_fitness(~all(isfinite(pop_fitness),2), :) = INVALID_FITNESS;
    ...
    rk = pareto_rank(pop_fitness); % 次世代に対するパレートランクを再計算（1が最良）—統計や代表抽出に使用
    ...
    if isscalar(front_idx) % 第1フロントが1個体なら距離比較せずその個体を採用
        ...
        cd_front = crowding_distance(F_front); % 第1フロント内のクラウディング距離を計算
    ...
    % ===== [F] ログ出力（CSV/デバッグ） =====
    log_write_generation(generation, pop_fitness, pop_vari, logdir, struct(...));
end

%% 07. 後処理と最終保存
% - 最終フロント抽出、メタ情報整理、results.mat の保存
rank_final = pareto_rank(pop_fitness); % 最終世代のパレートランクを計算（1が最良フロント）
...
meta.console = dbgfile; % デバッグサマリCSV（console相当）のパス
