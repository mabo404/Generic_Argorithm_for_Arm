function J = Evaluate(pop)
    ...
    Xe = [-0.5; -0.5; 0.6]; % 終点位置 [m]（x, y, z）
    r = T / T_total; % 正規化時間（0〜1）
    S = 10.*(r.^3) - 15.*(r.^4) + 6.*(r.^5); % 最小ジャークの時刻関数 S(r)
    Qstart = quaternion([-45, 0, 90], 'eulerd', 'ZYX', 'frame'); % 姿勢の始点：ZYX順オイラー角[deg]→四元数（intrinsic）
    QE = quaternion([-45 0 90],'eulerd','ZYX','frame'); % 姿勢の終点：始点と同一（一定姿勢を想定）

    %% 後で確認１ これで大丈夫？
    Qtraj_q = slerp(Qstart, QE, S); % S∈[0,1]に沿ってslerp補間（同姿勢なので実質一定）
    Qtraj_q = Qtraj_q(:); % 列ベクトル化し形状を統一（M×1）
    ...
    parfor i=1:n % 各個体を並列に評価（parfor）
        ...
        %% 後で確認３ これ本当にいる？
        if ~(all(isfinite(x0))) % 初期位置が有限値か確認（NaN/Inf対策）
            J(i,:) = invalid_row; % 不正なら無効評価で棄却
            continue; % 次の個体へ
        end
        ...
        [theta, info] = IK_solver(L, Ptraj, Qtraj_q(:), theta0); % 位置軌道Ptrajと姿勢Qtraj_qに対して全時刻の逆運動学を解く（初期角はtheta0）
        if isempty(theta) || ~isnumeric(theta) || size(theta,1)~=6 || size(theta,2)~=numel(T) || ~all(isfinite(theta(:))) % IK失敗/型不正/サイズ不一致/NaN・Inf混入の検査
            J(i,:) = invalid_row; % 異常時は当該個体を無効評価として棄却
            continue; % 次の個体へ
        end
        theta = unwrap(theta, [], 2); % 時系列方向（2次元目）で角度の位相を連続化（±2πの飛びを除去）
        if ~all(isfinite(theta(:))) % 位相連続化後もNaN/Infが無いかを再チェック
            J(i,:) = invalid_row; % 異常が残っていれば無効評価
            continue; % 次の個体へ
        end
        ...
        if ~all(isfinite(dth(:))) || ~all(isfinite(ddth(:))) % 速度・加速度にNaN/Infが含まれていないか検査
            J(i,:) = invalid_row; % 異常値があれば当該個体を無効評価にする
            continue; % 次の個体へ
        end
        ...
        for k = 1:M % すべての時刻ステップについて順に評価
            [p_k, q_k] = FK(L, theta(:,k)); % 正運動学：手先位置p_kと姿勢四元数q_kを算出
            if isempty(p_k) || ~all(isfinite([p_k(:); q_k(:)])) % 計算結果にNaN/Infや空が含まれていないか検査
                failed = true; break; % 異常があれば当該個体の評価を中断し失敗フラグ
            end
            ...
            pos_acc = pos_acc + sum((p_k - p_ref).^2); % 位置誤差の二乗を加算（時間積分用）
            q_ref = Qtraj_num(k,:); % 参照姿勢（四元数, 1×4）の取り出し
            if dot(q_ref, q_k) < 0, q_k = -q_k; end % antipodal対策：内積<0なら符号反転で最近傍回転に
            c = max(-1, min(1, dot(q_ref, q_k))); % 数値安定化のため内積を[-1,1]にクリップ
            phi = 2*acos(c); % 姿勢誤差角[rad]（四元数からの回転角）
        ...
        %% 後で確認６ これでいいの？
        J_pos = dt * pos_acc; % 位置誤差二乗の時間積分（台形ではなく矩形近似）
        J_ori = dt * ori_acc; % 姿勢誤差角二乗の時間積分

        torque = computeTorque(L, theta, ddth, m_joint); % トルク時系列を推定（動力学モデルに基づく）
        if isempty(torque) || ~isnumeric(torque) || ~all(isfinite(torque(:))) || size(torque,1) ~= 6
            J(i,:) = invalid_row; % 形式不正/NaN/Inf/関節数不一致なら無効評価
            continue;
        end
        ...
        d_min = minLinkDistance(L,theta); % 全時刻のリンク間最小距離を評価（自己干渉の指標）
        if isempty(d_min) || ~isfinite(d_min)  % 計算失敗やNaN/Infであれば当該個体を無効化
            J(i,:) = invalid_row;
            continue;
        end
        ...
        %% 後で確認８ こんなに確認いる？
        if ~util_isfinite(J(i,:))  % 計算結果にNaN/Infが混入していないか最終チェック
            J(i,:) = invalid_row; % 異常時は共通ペナルティ値に置換して当該個体を棄却
            continue; % 次の個体の評価へ移行
        end
    end
end